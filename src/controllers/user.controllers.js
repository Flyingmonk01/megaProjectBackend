...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (existedUser) {
        // If user already exists, delete uploaded files and return an error
        if (req.files) {
            Object.values(req.files).forEach((fileArray) => {
                fileArray.forEach((file) => {
                    fs.unlinkSync(file.path); // Delete the file from the temporary directory
                });
            });
        }
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    // check availibility of access token in user browsers
...
const refreshAccessToken = asyncHandler(async(req, res) => {
    ...
    try {
        const decodedToken = jwt.verify(incomingRefreshToken, process.env.JWT_TOKEN_ACCESS);
